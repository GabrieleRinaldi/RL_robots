...
class RewardsCfg:
    ...
    # bonus costante per ogni step vissuto (non terminato). Senza questo termine il reward per-step
    # medio è netto negativo (verificato su un run reale: iter20 mean_reward=-44.2 su 452 step ~ -0.10/step),
    # quindi PPO converge a "cadere il prima possibile" per minimizzare il reward totale accumulato -
    # è la causa del collasso a bad_orientation=100% osservato nei run precedenti. Peso scelto con margine
    # (~2x il deficit misurato) per rendere la sopravvivenza chiaramente conveniente; da ritarare se necessario.
